...
public class ReservationHandler {
	...
	public int cancelReservation(int id) {
		...
		try {
			session = factory.openSession();
			ts = session.beginTransaction();			
			// change reservation's state to 'handled yet'
			Reservation r = (Reservation) session.load(Reservation.class, id);
			r.setState(0);
			session.update(r);
			
			if(r.getEndTime() > System.currentTimeMillis()) {
				long diff;
				if(r.getStartTime() < System.currentTimeMillis()) {
					diff = r.getEndTime() - System.currentTimeMillis();
				} else {
					diff = r.getEndTime() - r.getStartTime();
				}
				if(diff > 1000) {
					User u = (User) session.createQuery(String.format(SQL.SELECT_USER_INFO, r.getUser_id())).uniqueResult();
					int lefttime = u.getLefttime();
					lefttime += (int) (diff / 1000);
					u.setLefttime(lefttime);
					session.update(u);
				}
			}
...
//	}
	...
	@SuppressWarnings("unchecked")
	public int handleReservations(long time) {
		Session session = null;
		Transaction ts = null;
		try {
			session = factory.openSession();
			ts = session.beginTransaction();
			// handle over reservations
			List<Reservation> list = session.createQuery(String.format(SQL.SELECT_OVER_RESERVATIONS, time, 1)).list();
			for(Reservation r : list) {
				Device d = (Device) session.createQuery(String.format(SQL.SELECT_DEVICE_INFO, r.getDevice_id())).uniqueResult();
				if(d.getState() == 2) { // device is reserved
			...
			// handle now reservations
			List<Reservation> list1 = session.createQuery(String.format(SQL.SELECT_NOW_RESERVATIONS, time, time)).list();
			...
			// handle actually no reservation
			List<Device> list2 = session.createQuery(String.format(SQL.SELECT_DEVICES_WITH_STATE, 2)).list();
			for(Device d : list2) {
				if((Reservation) session.createQuery(String.format(SQL.SELECT_DEVICE_NOW_RESERVATION, d.getId(), 1, time, time)).uniqueResult() == null) {
					d.setState(0);
					session.update(d);
				}
			}
